defmodule Blessd.Observance do
  ...
  def list_meetings(current_user) do
    with {:ok, query} <- Auth.check_user(Meeting, current_user) do
      query
      |> Meeting.preload()
      |> Meeting.order()
      |> Repo.list()
  ...
  def find_meeting(id, current_user) do
    with {:ok, query} <- Auth.check_user(Meeting, current_user), do: Repo.find(query, id)
  end
  ...
  def update_meeting(%Meeting{} = meeting, attrs, current_user) do
    with {:ok, meeting} <- Auth.check_user(meeting, current_user) do
      meeting
  ...
  def delete_meeting(%Meeting{} = meeting, current_user) do
    with {:ok, meeting} <- Auth.check_user(meeting, current_user), do: Repo.delete(meeting)
  end
  ...
  def find_occurrence(id, current_user) do
    with {:ok, query} <- Auth.check_user(MeetingOccurrence, current_user) do
      query
  ...
  def update_occurrence(%MeetingOccurrence{} = occurrence, attrs, current_user) do
    with {:ok, occurrence} <- Auth.check_user(occurrence, current_user) do
      occurrence
      |> MeetingOccurrence.changeset(attrs)
      |> Repo.update()
    end
  ...
  def delete_occurrence(%MeetingOccurrence{} = occurrence, current_user) do
    with {:ok, occurrence} <- Auth.check_user(occurrence, current_user), do: Repo.delete(occurrence)
  end
  ...
  def toggle_attendant(person_id, occurrence_id, current_user) do
    case Repo.find_by(Attendant, person_id: person_id, meeting_occurrence_id: occurrence_id) do
      {:ok, attendant} ->
        with {:ok, attendant} <- Auth.check_user(attendant, current_user), do: Repo.delete(attendant)
  ...
  def new_attendant(current_user) do
    Auth.check_user(%Attendant{church_id: current_user.church.id}, current_user)
  end
end
