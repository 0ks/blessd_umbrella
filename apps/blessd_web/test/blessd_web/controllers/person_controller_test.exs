defmodule BlessdWeb.PersonControllerTest do
  ...
  describe "index" do
    test "lists all people", %{conn: conn} do
      church = auth_church()
      conn = get(conn, person_path(conn, :index, church))
  ...
  describe "new person" do
    test "renders form", %{conn: conn} do
      church = auth_church()
      conn = get(conn, person_path(conn, :new, church))
  ...
  describe "create person" do
    test "redirects to index when data is valid", %{conn: conn} do
      church = auth_church()
      conn = post(conn, person_path(conn, :create, church), person: @create_attrs)
      assert redirected_to(conn) == person_path(conn, :index, church)
    end

    test "renders errors when data is invalid", %{conn: conn} do
      church = auth_church()
      conn = post(conn, person_path(conn, :create, church), person: @invalid_attrs)
      assert html_response(conn, 200) =~ "New Person"
    end
  end
  ...
  describe "edit person" do
    test "renders form for editing chosen person", %{conn: conn} do
      ...
      conn = get(conn, person_path(conn, :edit, church, person))
  ...
  describe "update person" do
    test "redirects when data is valid", %{conn: conn} do
      ...
      conn = put(conn, person_path(conn, :update, church, person), person: @update_attrs)
      assert redirected_to(conn) == person_path(conn, :index, church)
    ...
    test "renders errors when data is invalid", %{conn: conn} do
      ...
      conn = put(conn, person_path(conn, :update, church, person), person: @invalid_attrs)
      assert html_response(conn, 200) =~ "Edit Person"
    end
  end
  ...
  describe "delete person" do
    test "deletes chosen person", %{conn: conn} do
      ...
      conn = delete(conn, person_path(conn, :delete, church, person))
      assert redirected_to(conn) == person_path(conn, :index, church)

      assert_error_sent(404, fn ->
        get(conn, person_path(conn, :edit, church, person))
      end)
    end
  end
end
